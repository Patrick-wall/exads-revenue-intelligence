...
interface Client {
  ...
  pricing: Pricing[];
  plans: Partial<Record<Pricing, ClientPlan>>;
  platformStatus?: Partial<Record<Pricing, "active" | "testing" | "onboarding">>;
  sentiment: Sentiment;
...
const CLIENTS: Client[] = [
  ...
  { id:3, name:"DatingLeads", tradingName:"Adclix / Perfect Ads / Traffic Gold", vertical:["Adult","Dating"], language:"English", currency:"EUR", blurb:"Monetises member areas across a large portfolio of dating sites. Operates multiple brands including Adclix, Perfect Ads, and Traffic Gold.", rev:[3785,4114,3887,3084,3144,7320], adReqs:[1.3,1.4,1.3,1.1,1.1,2.5], cost:[340,380,350,285,290,680], tier:"T2", status:"active", pricing:["Ad Serving"], plans:{"Ad Serving":"Enterprise"}, sentiment:"happy", tickets:{open:2,resolved:4}, upsell:null },
  ...
  { id:14, name:"FlirtVentures", tradingName:"SSP / Former AdClix / Turtle Pace Media", vertical:["Adult","Dating"], language:"Dutch / English", currency:"EUR", blurb:"Netherlands-based dating network operating niche dating sites across Western Europe. Formerly operated as AdClix. Stable, consistent traffic.", rev:[1100,1050,980,1020,1045,1070], adReqs:[0.4,0.4,0.3,0.3,0.4,0.4], cost:[99,95,88,92,94,96], tier:"T3", status:"active", pricing:["Ad Serving"], plans:{"Ad Serving":"Enterprise"}, sentiment:"happy", tickets:{open:0,resolved:2}, upsell:"Pro plan upgrade" },
  ...
  return (
    ...
    >
      ...
      <td className="py-2.5 px-1 text-center">
        <span className="text-xs" title={client.vertical.join(", ")}>{client.vertical.map(v => VERTICAL_ICONS[v]).filter(Boolean).join(" ")}</span>
      </td>
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-slate-700/30 text-xs text-slate-500 uppercase tracking-wider">
                        ...
                        <th className="text-center px-1 py-2 font-medium" title="Mainstream \uD83C\uDF10 | Adult \uD83D\uDD1E | Dating \u2764\uFE0F | iGaming \uD83C\uDFB0">Vertical</th>
                ...
                {detail ? (
                  <div>
                    ...
                    <div className="ml-5 mb-3 flex items-center gap-1.5">
                      {detail.vertical.map(v => <span key={v} className="text-[10px] font-medium bg-slate-700/60 text-slate-300 px-2 py-0.5 rounded-full">{v}</span>)}
                    ...
                    {/* Platforms & Plans */}
                    <div className="mb-4">
                      <div className="text-xs text-slate-500 font-medium mb-2">Platforms</div>
                      <div className="space-y-2">
                        {detail.pricing.map(platform => {
                          const plan = detail.plans[platform] ?? "Core";
                          const pStatus = detail.platformStatus?.[platform] ?? (detail.status === "active" || detail.status === "warning" || detail.status === "critical" || detail.status === "declining" ? "active" : detail.status === "testing" ? "testing" : "onboarding");
                          return (
                            <div key={platform} className="bg-slate-900/50 rounded-lg px-3 py-2.5">
                              <div className="flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                  <span className={`w-1.5 h-1.5 rounded-full ${platform === "Ad Serving" ? "bg-cyan-500" : "bg-purple-500"}`} />
                                  <span className="text-xs text-slate-200 font-medium">{platform}</span>
                                  <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${
                                    pStatus === "active" ? "bg-emerald-500/15 text-emerald-400" :
                                    pStatus === "testing" ? "bg-violet-500/15 text-violet-400" :
                                    "bg-cyan-500/15 text-cyan-400"
                                  }`}>{pStatus}</span>
                                </div>
                                <div className="flex items-center gap-2">
                                  <span className="text-[10px] bg-slate-800 text-slate-300 px-2 py-0.5 rounded font-medium">{plan}</span>
                                  {plan !== "Enterprise" && (
                                    <span className="text-[10px] text-purple-400">
                                      {"\u2191"} {plan === "Core" ? "Pro" : plan === "Pro" ? "Business" : "Enterprise"}
                                    </span>
                                  )}
                                </div>
                              </div>
          ...
          {view === "reports" && (
            <div className="space-y-5">
              <div className="flex items-center gap-2 mb-1">
                ...
                <div className="text-sm text-slate-500">Auto-generated sales reports</div>
              </div>
